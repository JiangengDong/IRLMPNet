...
class Replay:
    ...
    def buffer_sample(self, n: int) -> Dict:
        ...
        return {"states": self.states[indices],
                ...
                "rewards": self.rewards[indices], # normalized reward
...
class TD3:
    def __init__(
        ...
        self.scheduler_actor = torch.optim.lr_scheduler.StepLR(self.optimizer_actor, 50000, 0.9)
        ...
        self.scheduler_critic = torch.optim.lr_scheduler.StepLR(self.optimizer_critic, 50000, 0.9)

        # define a replay buffer
        self.ReplayBuffer = Replay(buffer_size=500000, init_length=1000, state_dim=state_dim, action_dim=action_dim, env=env)
    ...
    def train(self, num_steps: int, log_dir: str) -> nn.Module:
        ...
        for iter in tqdm(range(num_steps)):
            ...
            if iter % 10000 == 0:
                torch.save(self.actor_target, model_filename + ".{}".format(iter)) # TODO: change this 
...
if __name__ == "__main__":
    ...
    model_filename = os.path.join("./data", "pytorch_model", "rl", "car_free-TD3-unnorm.pt")
    if task == "train":
        TD3_object = TD3(
            env=env,
            state_dim=6,
        ...
        # Train the policy
        log_dir = os.path.join("./data", "log", "rl", "car_free-TD3-unnorm-2")
        os.makedirs(os.path.split(model_filename)[0], exist_ok=True)
        policy = TD3_object.train(500000, log_dir)
        torch.save(policy, model_filename)

    policy = torch.load(model_filename).cuda()

    input_tensor = torch.zeros((1, 6)).cuda()
    policy_scripted = torch.jit.trace(policy, input_tensor)
    policy_scripted.save("data/pytorch_model/rl/car_free-TD3-unnorm-script.pt")
    # img_filename = os.path.join("./data", "img", "car", "TD3-unnorm-episode{}.gif")
    # os.makedirs(os.path.split(img_filename)[0], exist_ok=True)
