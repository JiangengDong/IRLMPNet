...
class DifferentialDrive1Order:
    ...
    # state-space-related parameters
    state_dim = 3  # [x, y, theta, v, omega]
    state_bound = np.array([
        [-2.0, 2.0],  # x range
        [-2.0, 2.0],  # y range
        [-1.0, 1.0],    # theta range
    ], dtype=np.float32)
    is_circular = np.array([False, False, True])
    ...
    def function(self, state: np.ndarray, control: np.ndarray) -> np.ndarray:
        ...
        deriv = np.zeros_like(state)
        deriv[0] = control[0] * np.cos(state[2]*np.pi)
        deriv[1] = control[0] * np.sin(state[2]*np.pi)
        deriv[2] = control[1]
        return deriv
