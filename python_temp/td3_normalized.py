...
import gym
from kino_env import Car1OrderEnv
...
class Actor(nn.Module):
    def __init__(self, state_dim: int, action_dim: int):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=16, kernel_size=(6, 6)),
            nn.PReLU(),
        ...
        self.fc1 = nn.Linear(6 + 64, 512)
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 2)

        self.fc1.weight.data.uniform_(-1 / np.sqrt(3+64), 1 / np.sqrt(3+64))
        self.fc2.weight.data.uniform_(-1 / np.sqrt(512), 1 / np.sqrt(512))
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        """
        pos, local_map = torch.split(state, (6, 11*11), dim=-1)
        pos = pos.view(-1, 6)
        local_map = local_map.view(-1, 1, 11, 11)
        hidden = self.encoder.forward(local_map)
        x = torch.cat([pos, hidden], dim=1)
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
        x = F.relu(x)
...
class TD3:
    def __init__(
        ...
        self.ReplayBuffer = Replay(buffer_size=100000, init_length=1000, state_dim=state_dim, action_dim=action_dim, env=train_env)
    ...
    def sample_action(self, state: np.ndarray, stochastic=True) -> np.ndarray:
        with torch.no_grad():
            ...
            action_mean = action_tensor.cpu().numpy().flatten()
        if stochastic:
...
if __name__ == "__main__":
    ...
    train_env = Car1OrderEnv()
    test_env = Car1OrderEnv()
    task = "train"
    ...
    if task == "train":
        ...
        # Train the policy
        log_dir = "data/car1order/tensorboard_log/RL"
        model_filename = "data/car1order/pytorch_model/rl/car1order-TD3-normalized.pt"
