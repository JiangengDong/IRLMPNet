...
namespace IRLMPNet
{
    ...
    std::vector<double> toVector(const torch::Tensor &tensor, const unsigned int dim) {
    ...
    /// The tensor is 2d, because the first dim is batch size. 
    torch::Tensor toTensor(const std::vector<double> &vec, const unsigned int dim) {
        std::vector<float> scaled_src(dim);
        for (unsigned int i = 0; i < dim; i++) {
            scaled_src[i] = vec[i];
        }
