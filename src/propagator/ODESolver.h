...
void addVec(const double *a, const double *b, const size_t dim, double *c) {
    for (size_t i = 0; i < dim; i++) {
        c[i] = a[i] + b[i];
    }
}
/// multiply a 5-dim vector with a scalar
void mulScalar(const double *a, const double b, const size_t dim, double *c) {
    for (size_t i = 0; i < dim; i++) {
        c[i] = a[i] * b;
    }
}

/// copy a 5-dim vector from source to destination
void copyVec(const double *a, const size_t dim, double *b) {
    for (size_t i = 0; i < dim; i++) {
        b[i] = a[i];
    }
}
...
void RK4(const double *state, const size_t state_dim, const double *control, const double dt, const ODEFunction ode, double *result) {
